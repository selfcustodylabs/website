...
- **No passphrase on either.** A [BIP39 passphrase](/docs/learn/keys/passphrase/) opens a different wallet entirely, and a stray space counts as one.

Then compare, in this order:

1. **The account xpub.** Both wallets should show the same [extended public key](/docs/learn/keys/xpub/) for the account. Compare the whole string, not the first few characters. This is the check that matters, because the xpub covers every address in the account rather than one of them.
...
Comparing addresses across devices protects the setup, not the coins. Once funded, the defence against any single compromised device is [multisig](/docs/learn/wallets/multisig/) across different vendors, where no one device can move funds on its own.

For this same comparison run against your finished metal backup rather than your paper draft, see [Backup Verification](/docs/wallet-setup/backup-verification/).
